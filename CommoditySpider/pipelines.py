...
from CommoditySpider.items import ClassifyItem, CommodityItem
...
class CommodityspiderPipeline(object):
    ...
    def save_classify(self, values):
        table = 'classify'
        keys = 'id, pid, title, url, type'
        sql = 'INSERT INTO {table}({keys}) VALUES {values} ON DUPLICATE KEY UPDATE title=VALUES(title), type=VALUES(type)'.format(
    ...
    def save_commodity(self, values):
        table = 'commodity'
        keys = 'id, cls_id, title, vendidos, month_vendidos, price_symbol, price, thumbnail, show_url'
